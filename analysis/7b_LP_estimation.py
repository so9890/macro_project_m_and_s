...
data = pd.read_pickle("../out_data_mngment/data_for_final_analysis_agg_cpi/data_inequality_agg_cpi")
data = data.sort_values(by=["year", "month"])
data.index = range(len(data))

path = "../out_figures/estimation_timeseries_agg_CPI/"
...
for c in data.columns[2:5]:
    ...
    plt.savefig(path+"time_series_of" + c, bbox_inches="tight")
    plt.clf()
...
plt.savefig(path+"real_ineq_stdev", bbox_inches="tight")
plt.clf()
...
plt.ylabel("Gini")
plt.xticks(data.loc[:157]["mm/yyyy"][::8], rotation=70)
plt.savefig(path+"real_ineq_Gini", bbox_inches="tight")
plt.clf()
...
plt.ylabel("p90-p10")
plt.xticks(data.loc[:157]["mm/yyyy"][::8], rotation=70)
plt.savefig(path+"real_ineq_p90-p10", bbox_inches="tight")
plt.clf()
...
exp_10 = pd.read_pickle("../out_data_mngment/data_for_final_analysis_agg_cpi/agg_cpi_exp_series_p10")
exp_10 = exp_10.sort_values(by=["year", "month"])
exp_10 = exp_10["exp_p1-p10_agg"]
exp_10.index = range(len(exp_10))
exp_10 = exp_10[:158]


exp_90 = pd.read_pickle("../out_data_mngment/data_for_final_analysis_agg_cpi/agg_cpi_exp_series_p90")
exp_90 = exp_90.sort_values(by=["year", "month"])
exp_90 = exp_90["exp_p90-p100_agg"]
exp_90.index = range(len(exp_90))
exp_90 = exp_90[:158]

# read in shock time series
shock_series = (
    pd.read_excel(
...
for k in [stdev, gini_coeff, p90_p10, exp_10, exp_90]:
    ...
    plt.ylabel(k.name)
    plt.savefig("../out_figures/estimation_timeseries_agg_CPI/" + k.name, bbox_inches="tight")
    plt.clf()