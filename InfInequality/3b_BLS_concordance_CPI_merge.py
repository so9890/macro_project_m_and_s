...
con_bls = pd.read_excel(
    "../../original_data/Concordance/concordance_BLS.xlsx", header=3, usecols="A:D"
)
...
dups = con_bls[
    con_bls.UCC.duplicated(keep=False)
...
# ------------------------------------------------------------------------
not_merged_in_con = d_CPI_test[
    ["source", "item_id_y", "UCC"]
]  # item_id_y is from con file
...
con_bls = con_bls.merge(
    con_exp_class, left_on="UCC", right_on="UCC", how="left", validate="1:1"
)
