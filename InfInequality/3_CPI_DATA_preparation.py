...
for i in range(0, len(unique_SID)):
    ...
    if len(regexIII.split(unique_SID["item_id"].iloc[i])) > 1:
        unique_SID["concordance_id"].iloc[i] = regexIII.split(
            unique_SID["item_id"].iloc[i]
        )[1]
    else:
...
NS_concordance = pd.read_excel(
    "../../original_data/Concordance/ELIconcordance_NS.xls",
    sheet_name=2,
    header=0,
    usecols="A:C",
)
...
NS_concordance[
    ...
    NS_concordance.eli98.duplicated()
)  # only second or higher order duplicate are indicated as true
print(
...
# test for duplicates
dups_con = unique_SID[unique_SID.concordance_id.duplicated(keep=False)].sort_values(
    ["concordance_id", "series_id"]
)
...
data = data.merge(
    unique_SID[["series_id", "item_id", "concordance_id"]],
    left_on="series_id",
    right_on="series_id",
    how="left",
    validate="m:1",
...
print(
    "There are no duplicates in the final CPI file in terms of concordance_id, year and period."
)
